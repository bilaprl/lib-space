...
export async function POST(request) {
  try {
    ...
    const { data: seat } = await supabase.from('seats').select('id, status, locked_by').eq('id', seatId).single();
    ...
    if (seat.locked_by) {
      await supabase.from('reservations')
        ...
        .eq('seat_id', seatId).eq('user_id', seat.locked_by).in('status', ['active']);
    }

    return Response.json({ message: `Kursi ${seatId} berhasil dikosongkan` });
  } catch (err) {
